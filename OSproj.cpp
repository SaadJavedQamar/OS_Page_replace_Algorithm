...
{
	cout<<"Enter Size of Data: ";             //Prompted for No. of pages OR Refrence strings size that are presented in secondary memory
	cin>>size;
	Array=new int[size];
	if(Algo_type==1)
	{
		//Automatic genrates Refrence strings
		cout<<"Enter Maximum Range of Reference String: ";  //prompted for Mazimun number in refrence strings
		cin>>max_range;    
	...
	{
		//Manually Enter Refrence strings
		cout<<"Enter Data You Want(Reference String): ";
		for(int i=0;i<size;i++)
		cin>>Array[i];
	}
	
	cout<<"Enter Frame Size: ";  //Prompted for number of frames in main memory in which pages will be move
	cin>>frame_size;
...
class Node
{
	...
	int data;
	Node *next;
	Node(int value=-1,Node *nood=NULL)
	{
		data=value;
		next=nood;
	}
};
//Using FIFO to for pages in and out
class Queue
{
	Node *head;
	public:
	...
	bool Check(int value)
	{
		...
		while(temp!=NULL)
		{
			if(temp->data==value)
			{
				return true;                      //If page is already present in frame then set the page and It is called page Hit 
			}
			temp=temp->next;
		}
		return false;
	...
	{
		if(head==NULL)
			return true; //page is absent in the frame
		return false; //page is present
	}
	void Remove(int value)          //page replacement function after 
	{
		if(head->data==value)
		{
			Node *temp=head;
...
class Stack
{
	...
	void Reset_Bit()
	{ 
	 BA[top%frame_size].bit=false; 
	}
	bool Get_Bit()
	{ 
	return BA[top%frame_size].bit; 
	}
	void Traverse()
	{
		for(int i=0;i<frame_size;i++)
...
class LRU                     //works as FIFO but replace the new page with first demand page which is use first in the frame
{
	int pfault;
	int hit;
	Queue q;
